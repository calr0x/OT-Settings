...
if [ -d "/root/OT-Settings" ]; then
    ...
    (cd /root/OT-Settings && git pull) 2>/dev/null && echo "${N1}"
else
...
if [ -d "/root/OT-NodeWatch" ]; then
    ...
    (cd /root/OT-NodeWatch && git pull) 2>/dev/null && echo "${N1}"
else
...
if [ -d "/root/OT-Smoothbrain-Backup" ]; then
    ...
    (cd /root/OT-Smoothbrain-Backup && git pull) 2>/dev/null && echo "${N1}"
else
